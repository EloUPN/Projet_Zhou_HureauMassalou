...
class Operations:
    ...
    def verifier_format_iban(df, colonne, valeur_remplacement='n/a'):
        """
        Vérifie et nettoie les IBAN - Accepte uniquement les IBAN français (FR) de 27 caractères
        ...
        # Convertir en string et nettoyer la notation scientifique
        df_clean[colonne] = df_clean[colonne].astype(str)
        df_clean[colonne] = df_clean[colonne].str.replace(',', '')  # Enlever virgules
        
        # Pattern IBAN français uniquement : FR + 2 chiffres + 23 caractères alphanumériques
        # Total : 27 caractères exactement
        pattern = r'^FR[0-9]{2}[A-Z0-9]{23}$'
        
        # Marquer comme invalides : ne correspond pas au pattern
        invalides = ~df_clean[colonne].str.match(pattern, na=False)
        invalides &= (df_clean[colonne] != '') & (df_clean[colonne] != 'nan') & (df_clean[colonne] != 'n/a')
        
        # Aussi invalider les IBAN étrangers (DE, GB, etc.) même s'ils sont bien formés
        etrangers = df_clean[colonne].str.match(r'^(?!FR)[A-Z]{2}[0-9]', na=False)
        invalides |= etrangers
        
        # Invalider la notation scientifique
        scientifique = df_clean[colonne].str.contains('E\\+|e\\+|E-|e-', na=False)
        invalides |= scientifique
    ...
    def verifier_format_siret(df, colonne, valeur_remplacement='n/a'):
        """
        Vérifie les numéros SIRET (14 chiffres exactement)
        ...
        # Convertir en string et nettoyer la notation scientifique/virgules
        df_clean[colonne] = df_clean[colonne].astype(str)
        df_clean[colonne] = df_clean[colonne].str.replace(',', '')  # Enlever virgules
        ...
        # Invalider la notation scientifique (1,23E+16)
        scientifique = df_clean[colonne].str.contains('E\\+|e\\+|E-|e-', na=False)
        invalides |= scientifique
        
        # Invalider si trop court ou trop long
        longueur_incorrecte = df_clean[colonne].str.len() != 14
        invalides |= (longueur_incorrecte & (df_clean[colonne] != '') & (df_clean[colonne] != 'nan') & (df_clean[colonne] != 'n/a'))
