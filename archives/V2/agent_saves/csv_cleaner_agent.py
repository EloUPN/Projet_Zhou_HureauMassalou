...
# -*- coding: utf-8 -*-
"""
Agent de nettoyage CSV avec Mistral AI
Usage: python csv_cleaner_mistral.py mon_fichier.csv
"""
...
class MistralCSVCleanerAgent:
    def __init__(self, api_key):
        """Initialise l'agent avec la clé API Mistral"""
        self.api_key = api_key
        self.api_url = "https://api.mistral.ai/v1/chat/completions"
        self.model = "mistral-small-latest"  # Modèle gratuit et performant
    ...
    def analyze_csv(self, filepath):
        ...
        print("\n🔍 Analyse des anomalies en cours avec Mistral AI...")
        
        # Demander à Mistral d'analyser
...
"""
        ...
        return self._parse_json_response(response), df
    
    def propose_corrections(self, anomalies_json, df):
        """Propose des corrections pour les anomalies détectées"""
        print("\n💡 Génération des propositions de correction avec Mistral AI...")
...
def main():
    """Fonction principale"""
    print("=" * 60)
    print("🤖 AGENT DE NETTOYAGE CSV AVEC MISTRAL AI")
    print("=" * 60)
    
    # Vérifier les arguments
    if len(sys.argv) < 2:
        print("\n❌ Usage : python csv_cleaner_mistral.py fichier.csv")
        sys.exit(1)
    ...
    if not api_key:
        ...
        print("💡 Exécutez : set MISTRAL_API_KEY=votre_cle_api")
        print("💡 Obtenez votre clé gratuite sur : https://console.mistral.ai/")
        sys.exit(1)
    
    # Créer l'agent
    agent = MistralCSVCleanerAgent(api_key)
