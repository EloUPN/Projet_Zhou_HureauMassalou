...
class Operations:
    ...
    def standardiser_format_date(df, colonne, format_sortie='%Y-%m-%d'):
        ...
        for fmt in formats_a_tester:
            # On ne tente la conversion que pour les lignes encore vides (NaT)
            masque_manquant = dates_converties.isna()
            if not masque_manquant.any():
                break
                
            try:
                # Tentative de conversion sur les valeurs restantes
        ...
        if conversions > 0:
            # On conserve les valeurs d'origine pour les échecs (au lieu de supprimer)
            # On ne formate QUE les dates réussies
            masque_valides = dates_converties.notna()
            
            # Créer une copie de la colonne d'origine pour ne pas perdre de données
            resultat = df_clean[colonne].astype(str).copy()
            
            # Remplacer uniquement les dates reconnues par le nouveau format
