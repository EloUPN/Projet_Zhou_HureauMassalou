...
3. Emails INVALIDES (sans @ ou sans .com/.fr/etc.) → remplacer par "n/a"
4. Valeurs MANQUANTES (cellules vides) → NE PAS TOUCHER, les laisser vides
5. Doublons → supprimer les lignes entières en double
6. Espaces inutiles → nettoyer
7. Noms de colonnes → normaliser
8. Colonnes comme code_postal, telephone, numero_employe → FORCER en type texte (forcer_type_texte)

DÉTECTION OBLIGATOIRE :
- Colonnes "code_postal", "telephone", "numero_employe", "code_*" → TOUJOURS proposer forcer_type_texte
- Emails sans @ ou sans extension (.com, .fr, etc.) → remplacer_emails_invalides
- Âge : < 0 ou > 120 → remplacer par "n/a"
- Salaire : < 0 ou très élevé → remplacer par "n/a"
- Heures travail : < 0 ou > 70 → remplacer par "n/a"
- Codes postaux : invalides (longueur incorrecte) → remplacer par "n/a"
- Téléphones : invalides (trop court/long) → remplacer par "n/a"

NE PAS PROPOSER remplir_valeurs_manquantes (sauf si explicitement demandé)

ORDRE DE PRIORITÉ DES OPÉRATIONS :
1. forcer_type_texte (TOUJOURS EN PREMIER pour code_postal, telephone, etc.)
2. supprimer_doublons
3. nettoyer_espaces
4. remplacer_valeurs_invalides / remplacer_valeurs_aberrantes
5. remplacer_emails_invalides
6. normaliser_noms_colonnes

Ta tâche :
1. Identifier toutes les anomalies
2. Pour CHAQUE anomalie, proposer l'opération appropriée
3. TOUJOURS proposer forcer_type_texte pour les colonnes de codes/numéros
4. TOUJOURS utiliser "n/a" pour remplacer les valeurs aberrantes/invalides
5. NE JAMAIS proposer de remplir les valeurs manquantes
...
EXEMPLES CORRECTS :
- Colonne code_postal existe → {{"operation": "forcer_type_texte", "parametres": {{"colonnes": ["code_postal", "telephone", "numero_employe"]}}}}
- email="test@invalid" (sans extension) → {{"operation": "remplacer_emails_invalides", "parametres": {{"colonne": "email", "valeur_remplacement": "n/a"}}}}
- age=-5 → {{"operation": "remplacer_valeurs_invalides", "parametres": {{"colonne": "age", "valeur_remplacement": "n/a", "min_valeur": 0, "max_valeur": 120}}}}
- heures_semaine=999 → {{"operation": "remplacer_valeurs_invalides", "parametres": {{"colonne": "heures_semaine", "valeur_remplacement": "n/a", "min_valeur": 0, "max_valeur": 70}}}}

IMPORTANT : 
- TOUJOURS détecter les colonnes code_postal, telephone, numero_employe et proposer forcer_type_texte
