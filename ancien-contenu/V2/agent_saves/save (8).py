...
7. Si colonne date avec formats variés (DD/MM/YYYY, DD-MM-YYYY) → standardiser_format_date
8. Si colonne date existe → detecter_incoherences_dates (futures, < 1900)
...
14. Salaire : < 0 ou > 200000 (aberrant) ou valeurs 999/999999 → remplacer_valeurs_invalides ou remplacer_valeurs_aberrantes
15. Heures : <= 0 ou > 70 → remplacer_valeurs_invalides (heures_semaine doit être >= 1)
...
7. standardiser_format_date (si colonne date existe) - ID: 7
8. detecter_incoherences_dates (si colonne date existe) - ID: 8
...
- ID 6 : {{"operation": "standardiser_format_date", "parametres": {{"colonne": "date_embauche"}}}}
- ID 7 : {{"operation": "detecter_incoherences_dates", "parametres": {{"colonne_date": "date_embauche", "valeur_remplacement": "n/a"}}}}
...
- ID 12 : {{"operation": "remplacer_valeurs_aberrantes", "parametres": {{"colonne": "salaire", "valeur_remplacement": "n/a", "methode": "iqr"}}}}
...
- TOUJOURS proposer standardiser_format_date si colonne date existe avec formats variés
- TOUJOURS proposer detecter_incoherences_dates si colonne date existe
- NE JAMAIS toucher aux valeurs manquantes"""
    ...
    def _executer_operation(self, df, operation_nom, parametres):
        ...
        elif operation_nom == 'supprimer_accents':
            colonnes = parametres.get('colonnes', None)
            return self.operations.supprimer_accents(df, colonnes)
        
        elif operation_nom == 'verifier_format_iban':
            colonne = parametres.get('colonne')
            valeur_remplacement = parametres.get('valeur_remplacement', 'n/a')
            return self.operations.verifier_format_iban(df, colonne, valeur_remplacement)
        
        elif operation_nom == 'verifier_format_siret':
            colonne = parametres.get('colonne')
            valeur_remplacement = parametres.get('valeur_remplacement', 'n/a')
            return self.operations.verifier_format_siret(df, colonne, valeur_remplacement)
        
        elif operation_nom == 'detecter_incoherences_dates':
            colonne_date = parametres.get('colonne_date') or parametres.get('colonne')
            valeur_remplacement = parametres.get('valeur_remplacement', 'n/a')
            return self.operations.detecter_incoherences_dates(df, colonne_date, valeur_remplacement)
