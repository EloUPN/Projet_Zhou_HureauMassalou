...
class AgentCSV:
    ...
    def analyser_csv(self, filepath):
        ...
        # Colonnes à forcer en texte dès la lecture (évite notation scientifique)
        colonnes_texte = ['code_postal', 'telephone', 'numero_employe', 'iban', 'siret', 
                          'numero_client', 'numero_commande', 'code_article']
        
        try:
            # Lire d'abord sans dtype pour détecter les colonnes
            df_temp = pd.read_csv(filepath, encoding='utf-8', nrows=0)
            colonnes_presentes = df_temp.columns.tolist()
            
            # Créer dtype dict pour colonnes à forcer en texte
            dtype_dict = {}
            for col in colonnes_presentes:
                col_lower = col.lower()
                if any(mot in col_lower for mot in colonnes_texte):
                    dtype_dict[col] = str
            
            # Relire avec les bons types
            df = pd.read_csv(filepath, encoding='utf-8', dtype=dtype_dict, keep_default_na=False)
            # Remplacer '' par NaN pour les vraies valeurs manquantes
            df = df.replace('', pd.NA)
            
        except UnicodeDecodeError:
            try:
                df_temp = pd.read_csv(filepath, encoding='latin-1', nrows=0)
                colonnes_presentes = df_temp.columns.tolist()
                dtype_dict = {}
                for col in colonnes_presentes:
                    col_lower = col.lower()
                    if any(mot in col_lower for mot in colonnes_texte):
                        dtype_dict[col] = str
                df = pd.read_csv(filepath, encoding='latin-1', dtype=dtype_dict, keep_default_na=False)
                df = df.replace('', pd.NA)
            except:
                df_temp = pd.read_csv(filepath, encoding='cp1252', nrows=0)
                colonnes_presentes = df_temp.columns.tolist()
                dtype_dict = {}
                for col in colonnes_presentes:
                    col_lower = col.lower()
                    if any(mot in col_lower for mot in colonnes_texte):
                        dtype_dict[col] = str
                df = pd.read_csv(filepath, encoding='cp1252', dtype=dtype_dict, keep_default_na=False)
                df = df.replace('', pd.NA)
...
1. Si colonnes code_postal, telephone, numero_employe, iban, siret existent → TOUJOURS proposer forcer_type_texte EN PREMIER (empêche notation scientifique Excel)
...
5. Si colonne iban existe → TOUJOURS proposer verifier_format_iban APRÈS forcer_type_texte (rejeter IBAN non français, longueur ≠ 27)
6. Si colonne siret existe → TOUJOURS proposer verifier_format_siret APRÈS forcer_type_texte (longueur exacte = 14 chiffres)
...
1. forcer_type_texte (code_postal, telephone, numero_employe, iban, siret) - ID: 1 - CRITIQUE pour éviter notation scientifique
...
- ID 1 : {{"operation": "forcer_type_texte", "parametres": {{"colonnes": ["code_postal", "telephone", "numero_employe", "iban", "siret"]}}}}
...
IMPORTANT : 
- RESPECTER L'ORDRE : forcer_type_texte EN PREMIER (avec iban, siret), puis supprimer_accents, puis validations
- TOUJOURS détecter code_postal, telephone, numero_employe, iban, siret pour forcer_type_texte
